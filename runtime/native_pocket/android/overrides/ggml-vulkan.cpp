...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_aii_matmul_f32 {};
    ...
    vk_pipeline pipeline_sigmoid_strided[2];
    ...
    vk_pipeline pipeline_col2im_1d_f32, pipeline_col2im_1d_f16;
...
static void ggml_vk_load_shaders(vk_device& device) {
    ...
    if (device->coopmat2) {
        ...
        // Only regular non-quantized matmul uses these configurations. Keep
        // quantized/ID and cooperative paths unchanged and separately gated.
        const uint32_t scalar_warp = aii_scalar_tile_warp(device->subgroup_size, device->coopmat_support);
        l_warptile = { 128,                         128, 128, 16, scalar_warp * 2, 64, 2, tm_l, tn_l, tk_l, scalar_warp };
        m_warptile = { 128,                          64,  64, 16, scalar_warp,     32, 2, tm_m, tn_m, tk_m, scalar_warp };
        s_warptile = { std::max(scalar_warp, 32u),    32,  32, 16, s_warptile_wm,  32, 2, tm_s, tn_s, tk_s, scalar_warp };
    ...
    // Explicit development selection; no released runtime default changes.
    if (getenv("AII_VK_F32_TILED") != nullptr) {
        ggml_vk_create_pipeline(device, device->pipeline_aii_matmul_f32,
            "aii_matmul_f32_tiled", sizeof(aii_matmul_f32_spv), aii_matmul_f32_spv,
            "main", 3, sizeof(vk_mat_mat_push_constants), {32, 8, 1}, {}, 1);
    }
...
#undef CREATE_BINARY
    ...
    ggml_vk_create_pipeline(device, device->pipeline_pad_reflect_1d_f32, "pad_reflect_1d_f32", pad_reflect_1d_f32_len, pad_reflect_1d_f32_data, "main", 2, sizeof(vk_op_pad_push_constants), {512, 1, 1}, {}, 1);
...
#define CREATE_UNARY(name)  \
    ...
    ggml_vk_create_pipeline(device, device->pipeline_sigmoid_strided[0], "sigmoid_strided_f32", sigmoid_strided_f32_len, sigmoid_strided_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {512, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_sigmoid_strided[1], "sigmoid_strided_f16", sigmoid_strided_f16_len, sigmoid_strided_f16_data, "main", 2, sizeof(vk_op_unary_push_constants), {512, 1, 1}, {}, 1);
...
static void ggml_vk_ctx_end(vk_context& ctx) {
    ...
    // UMA readback below directly reads a host-coherent mapping. Coherence
    // removes invalidateMappedMemoryRanges, not the GPU-write -> host-read
    // memory dependency. A fence alone supplies execution completion. See
    // Khronos synchronization_examples: CPU read back of compute output.
    const bool transfer_only = ctx->p->q->transfer_only;
    const vk::AccessFlags writes = transfer_only ? vk::AccessFlagBits::eTransferWrite :
        (vk::AccessFlagBits::eShaderWrite | vk::AccessFlagBits::eTransferWrite);
    ctx->s->buffer->buf.pipelineBarrier(
        ctx->p->q->stage_flags, vk::PipelineStageFlagBits::eHost, {},
        { vk::MemoryBarrier{writes, vk::AccessFlagBits::eHostRead} }, {}, {});
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    ...
    if (ctx->device->pipeline_aii_matmul_f32 && src0->type == GGML_TYPE_F32 &&
        src1->type == GGML_TYPE_F32 && !qx_needs_dequant && !qy_needs_dequant && !quantize_y) {
        pipeline = ctx->device->pipeline_aii_matmul_f32;
    }
...
static void ggml_vk_mul_mat(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    } else if (!(ctx->device->pipeline_aii_matmul_f32 && src0->type == GGML_TYPE_F32 &&
                 src1->type == GGML_TYPE_F32 && ggml_vk_dim01_contiguous(src0) && ggml_vk_dim01_contiguous(src1)) &&
        ...
        switch (ggml_get_unary_op(dst)) {
            ...
            case GGML_UNARY_OP_SIGMOID:
                if (!ggml_is_contiguous(src0) || !ggml_is_contiguous(dst)) {
                    return ctx->device->pipeline_sigmoid_strided[dst->type == GGML_TYPE_F16];
                }
...
static void ggml_vk_op_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst, ggml_op op, PC&& pc) {
    ...
    } else if (op == GGML_OP_IM2COL || op == GGML_OP_IM2COL_FAST_1D || op == GGML_OP_IM2COL_3D) {
...
static void ggml_vk_col2im_1d(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst) {
    ...
    const uint32_t K = K_OC / (uint32_t) OC;
    ...
    ggml_vk_op_f32<vk_op_col2im_1d_push_constants>(ctx, subctx, src0, nullptr, nullptr, nullptr, dst, GGML_OP_COL2IM_1D, {
        T_in, T_out, (uint32_t) OC, K, K_OC, s0, p0,
    });
}
...
static void ggml_vk_synchronize(ggml_backend_vk_context * ctx) {
    ...
    if (ctx->submit_pending) {
        // A real completion command carries the GPU-write -> host-read
        // dependency and the fence together. On the physical Pixel, the
        // former zero-command submission returned while mapped output was
        // still changing. No sleep, second-read retry or queue-wide idle is
        // used to conceal that failure: the immediate read remains the gate.
        auto completion = ggml_vk_create_temporary_context(ctx->compute_cmd_pool);
        ggml_vk_ctx_begin(ctx->device, completion);
        auto * completion_cmd = completion->s->buffer;
        if (ctx->device->async_use_transfer_queue && ctx->transfer_semaphore_last_submitted < ctx->transfer_semaphore.value) {
            completion->s->wait_semaphores.push_back(ctx->transfer_semaphore);
            ctx->transfer_semaphore_last_submitted = ctx->transfer_semaphore.value;
        }
        ggml_vk_ctx_end(completion);
        ggml_vk_submit(completion, ctx->fence);
        ggml_vk_wait_for_fence(ctx);
        completion_cmd->in_use = false;
        completion_cmd->buf.reset();
...
static bool ggml_vk_can_fuse(const ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph, int node_idx, std::initializer_list<enum ggml_op> ops) {
    // The alternate F32 kernel computes only matmul, never an adjacent bias.
    // Keep the actual add node rather than silently dropping a fused result.
    const auto *first = cgraph->nodes[node_idx];
    if (ctx->device->pipeline_aii_matmul_f32 && first->op == GGML_OP_MUL_MAT &&
        first->src[0]->type == GGML_TYPE_F32 && first->src[1]->type == GGML_TYPE_F32 &&
        ggml_vk_dim01_contiguous(first->src[0]) && ggml_vk_dim01_contiguous(first->src[1])) {
        return false;
    }
...
static bool ggml_backend_vk_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    const bool uses_bda = (op->op == GGML_OP_IM2COL || op->op == GGML_OP_IM2COL_FAST_1D || op->op == GGML_OP_IM2COL_3D) &&
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ...
    for (int f = 0; f < ctx->num_additional_fused_ops + 1; ++f) {
        ...
        } else if (tensor->op == GGML_OP_IM2COL || tensor->op == GGML_OP_IM2COL_FAST_1D) {
