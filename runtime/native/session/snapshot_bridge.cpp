...
Json SnapshotBridge::page(uint64_t offset,bool digest,Clock::time_point deadline,Store store) {
  auto q=object();put(q,"offset",number(offset));put(q,"digest",boolean(digest));
  return exchange(std::move(q),deadline,store);
}
Json SnapshotBridge::exchange(Json query,Clock::time_point deadline,Store store) {
...
std::string SnapshotBridge::read(bool* absent,Store store) {
  acquire();try{auto bytes=read_owned(absent,Clock::now()+std::chrono::seconds(10),store);release();return bytes;}
  catch(...){release();throw;}
}
std::string SnapshotBridge::read_owned(bool* absent,Clock::time_point deadline,Store store) {
  const auto limit=store_limit(store);
  if(absent)*absent=false;
  std::string bytes,expected;uint64_t size=0;
  do {
    auto result=page(bytes.size(),bytes.empty(),deadline,store);const auto* p=field(result.get(),"value");
    ...
    require(total>0,"empty snapshot is not an empty enrollment set");
    ...
    require(!decoded.empty()&&decoded.size()==integer(field(p,"bytes"),65536)&&decoded.size()<=size-bytes.size(),"snapshot page extent differs");
  ...
  auto check=page(size,true,deadline,store);const auto* p=field(check.get(),"value");
...
Json SnapshotBridge::publish(const std::string& candidate,const std::string& expected,bool absent,const std::string& upload,Store store) {
  const auto limit=store_limit(store);
  ...
  try {
    ...
    for(size_t offset=0;offset<candidate.size();offset+=65536) {
      ...
      auto receipt=exchange(std::move(q),deadline,store);require(!field(receipt.get(),"error"),"enrollment upload refused; no publication requested");
    ...
    publication_sent=true;auto receipt=exchange(std::move(q),deadline,store);
    ...
    require(read_owned(nullptr,deadline,store)==candidate,"published enrollment readback differs");
