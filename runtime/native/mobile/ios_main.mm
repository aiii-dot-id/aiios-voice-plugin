...
int aii_mobile_tts_fixture_main(int,char**);

static bool VerifyInputs(NSString* root, NSUInteger expected=33) {
...
- (BOOL)application:(UIApplication*)app didFinishLaunchingWithOptions:(NSDictionary*)options {
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED,0),^{
    ...
    const bool ttsMode=requestedMode && std::string(requestedMode)=="tts";
    if(freopen(log.fileSystemRepresentation,"wx",stdout) && freopen(errors.fileSystemRepresentation,"wx",stderr)) {
      ...
      if(![backend isEqualToString:@"cpu"] && ![backend isEqualToString:@"metal"])return;
      ...
      const bool tts=mode && std::string(mode)=="tts";
      if(mode && std::string(mode)!="tts" && std::string(mode)!="conversation")return;
      NSString* ttsInputs=[documents stringByAppendingPathComponent:@"tts-fixture"];
      if(tts)strings={"AiiVoiceMobile",[root stringByAppendingPathComponent:@"models/tts"].fileSystemRepresentation,
          ttsInputs.fileSystemRepresentation,[output stringByAppendingPathComponent:@"audio"].fileSystemRepresentation,backend.UTF8String};
      std::vector<char*> argv;for(auto& s:strings)argv.push_back(s.data());
      if(VerifyInputs(root) && (!tts || VerifyInputs(ttsInputs,9)))
        rc=tts?aii_mobile_tts_fixture_main(int(argv.size()),argv.data()):aii_mobile_fixture_main(int(argv.size()),argv.data());
