...
const evidence = { scope: 'Chrome Web Audio + shipped voice module + actual host app/SDK/real models; recorded input and fixed reply text; no physical audio or LLM quality claim',
...
function json(message) {
  ...
  if(window.proofContract?.withholdTerminalReceipt && message.voice?.playback?.terminal && message.voice.playback.stream===4) {
...
async function main() {
  ...
  socket.onmessage=ev=>{
    try {
      ...
      if(m.type==='voice_event') {
        ...
        if(m.voice_event.type==='failure') {
          const el=document.getElementById('voice-transcript');
          evidence.engine_failure_display={text:el.textContent,failed:el.classList.contains('failed'),hidden:el.hidden,
            session_id:m.voice_event.session_id,reason:m.voice_event.reason};
        }
  ...
  check(evidence.events.some(e=>e.type==='transcript_partial' && e.at<finals[0].at),
    'no actual partial reached the browser before the first final');
  ...
  check(eventCount('failed')===0 && eventCount('failure')===0 && eventCount('host_error')===0,'host or engine failure');
  ...
  for(const stream of ['2','4']) {
    ...
    const expectedSamples=window.proofContract?.expectedReplySamples ?? 115200;
    check(st && st.EngineReceived===expectedSamples && st.Written===expectedSamples*2 && st.Refused===0 && st.Ended,
...
main().catch(err=>{evidence.passed=false;evidence.error=String(err.stack||err);}).finally(async()=>{
  ...
  if(!evidence.host_readback) {
    try { const r=await fetch('/__readback'); if(r.ok) evidence.host_readback=await r.json(); }
    catch(err) { evidence.readback_error=String(err); }
  }
