...
func TestRealResidentEngineTwoRepliesReachHostSink(t *testing.T) {
	...
	for _, rate := range []int{24000, 48000} {
		t.Run(fmt.Sprintf("speaker_%d", rate), func(t *testing.T) {
			...
			report := map[string]any{
				...
				"host_revision": "45025460734667e2da9c44d9ad44a495bb54b86f",
			...
			for _, path := range []string{
				...
				".build/aii-voice-t3-race", "plugin/integration/host_resident_output_test.go",
				"runtime/plugin_engine/session.py", "runtime/plugin_engine/worker.py", "runtime/plugin_engine/readiness.py",
			} {
			...
			if err := plane.Register(&audio.Endpoint{ID: "mic", Source: &residentOutputSource{}}); err != nil {
				t.Fatal(err)
			}
